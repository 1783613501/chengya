...
<script>
import config from "../../../public/config/RebateManage/taskEdit";
export default {
  name: "ShopEdit",
  components: {
  ...
  mounted() {
    ...
    if (this.$route.query.type == "新增活动") {
      ...
      this.config[0].row[0].data[0].data.text = "新增活动";
    ...
    if (this.$route.query.type == "查看") {
      ...
      this.config[0].row[0].data[0].data.text = "查看活动";
    }
    if (this.$route.query.type == "编辑") {
      ...
      this.config[0].row[0].data[0].data.text = "编辑活动";
      this.config[0].row[0].data[1].data[0].form[0].disabled = true;
      this.config[0].row[0].data[1].data[0].form[0].readonly = true;
    }
  ...
  computed: {
    detail() {
      console.log("computer-----------------------------", this.$store.state.RebateManage.detail);
      return this.$store.state.RebateManage.detail;
    },
  },
  watch: {
