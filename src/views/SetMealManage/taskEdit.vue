...
<script>
import config from "../../../public/config/SetMealManage/taskEdit";
export default {
  name: "ShopEdit",
  components: {
  ...
  mounted() {
    ...
    if (this.$route.query.type == "新增套餐") {
      this.config[0].row[0].data[1].data[0].form.forEach((element) => {
        if(element.value != '销售员' && element.value != '适用商家'){
            element.disabled = false;
            element.readonly = false;
        }
      });
      this.config[0].row[0].data[0].data.text = "新增套餐";
    ...
    if (this.$route.query.type == "查看") {
      ...
      this.config[0].row[0].data[0].data.text = "查看套餐";
    }
    if (this.$route.query.type == "编辑") {
      ...
      this.config[0].row[0].data[0].data.text = "编辑套餐";
      this.config[0].row[0].data[1].data[0].form[0].disabled = true;
      this.config[0].row[0].data[1].data[0].form[0].readonly = true;
    }
  ...
  computed: {
    detail() {
      console.log("computer-----------------------------", this.$store.state.SetMealManage.detail);
      return this.$store.state.SetMealManage.detail;
    },
  },
  watch: {
